...
<head>
    ...
    <title>Order #{{ str_pad($order->number, 5, '0', STR_PAD_LEFT) }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: DejaVu Sans, Arial, sans-serif; font-size: 11px; color: #222; background: #fff; }

        /* ── Header ── */
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 28px; border-bottom: 2px solid #047857; padding-bottom: 16px; }
        .company-info h1 { font-size: 18px; font-weight: 700; color: #047857; margin-bottom: 4px; }
        .company-info p { font-size: 10px; color: #555; line-height: 1.5; }
        .logo { max-height: 64px; max-width: 160px; }
        .doc-meta { text-align: right; }
        .doc-meta h2 { font-size: 16px; font-weight: 700; text-transform: uppercase; color: #047857; margin-bottom: 6px; }
        ...
        table.lines th { background: #047857; color: #fff; font-size: 9px; text-transform: uppercase; letter-spacing: .05em; padding: 6px 8px; text-align: left; }
        ...
        table.totals tr.grand td { border-top: 2px solid #047857; font-size: 13px; font-weight: 800; color: #047857; padding-top: 8px; }
    ...
    <div class="info-grid">
        <div class="info-box">
            ...
            <p>
                ...
                @if($order->client->address){{ $order->client->address }}<br>@endif
                @if($order->client->postal_code || $order->client->locality)
                    {{ $order->client->postal_code }} {{ $order->client->locality }}<br>
                @endif
                @if($order->client->nif) NIF: {{ $order->client->nif }} @endif
            </p>
        </div>
        <div class="info-box" style="flex:0 0 200px;">
            ...
            <p>
                ...
                @if($order->proposal_id)
                    <br>From Proposal: #{{ $order->proposal_id }}
                @endif
            </p>
        </div>
    </div>
