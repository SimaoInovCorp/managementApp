...
<head>
    ...
    <title>Proposal #{{ str_pad($proposal->number, 5, '0', STR_PAD_LEFT) }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .doc-meta h2 { font-size: 16px; font-weight: 700; text-transform: uppercase; color: #1a56db; margin-bottom: 6px; }
        ...
        table.lines th { background: #1a56db; color: #fff; font-size: 9px; text-transform: uppercase; letter-spacing: .05em; padding: 6px 8px; text-align: left; }
        ...
        table.totals tr.grand td { border-top: 2px solid #1a56db; font-size: 13px; font-weight: 800; color: #1a56db; padding-top: 8px; }
...
<body>
    ...
    <div class="header">
        ...
        <div class="doc-meta">
            <h2>Proposal</h2>
            <div class="number">#{{ str_pad($proposal->number, 5, '0', STR_PAD_LEFT) }}</div>
            <p>Date: <strong>{{ $proposal->proposal_date->format('d/m/Y') }}</strong></p>
            <p>Valid until: <strong>{{ $proposal->validity_date->format('d/m/Y') }}</strong></p>
    ...
    <div class="info-grid">
        <div class="info-box">
            ...
            <p>
                ...
                @if($proposal->client->address){{ $proposal->client->address }}<br>@endif
                @if($proposal->client->postal_code || $proposal->client->locality)
                    {{ $proposal->client->postal_code }} {{ $proposal->client->locality }}<br>
                @endif
                @if($proposal->client->nif) NIF: {{ $proposal->client->nif }} @endif
            </p>
        </div>
        <div class="info-box" style="flex:0 0 200px;">
            <h4>Reference</h4>
            <p>
                Proposal No.: <strong>{{ str_pad($proposal->number, 5, '0', STR_PAD_LEFT) }}</strong><br>
                Date: {{ $proposal->proposal_date->format('d/m/Y') }}<br>
                Valid until: {{ $proposal->validity_date->format('d/m/Y') }}
            </p>
        </div>
    </div>
    ...
    <div class="footer">
        ...
        @if($company?->tax_number)— NIF {{ $company->tax_number }} @endif
        — This proposal is valid until {{ $proposal->validity_date->format('d/m/Y') }}.
